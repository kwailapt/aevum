...
pub async fn dispatch(req: McpRequest) -> McpResponse {
    match req.method.as_str() {
        "tools/call" => dispatch_tool_call(req).await,
...
async fn dispatch_tool_call(req: McpRequest) -> McpResponse {
    let tool_name = req.params.get("name").and_then(|v| v.as_str()).unwrap_or("");
    let args = req.params.get("arguments").cloned().unwrap_or(Value::Null);

    match tool_name {
        "aevum_remember" => crate::tools::remember::handle(req.id, args).await,
        "aevum_recall"   => crate::tools::recall::handle(req.id, args).await,
        "aevum_filter"   => crate::tools::filter::handle(req.id, args).await,
        "aevum_settle"   => crate::tools::settle::handle(req.id, args).await,
