...
// HTTP transport: axum on :8889, MCP Streamable HTTP 2025-03-26 compliant.
//
// Endpoints:
//   POST /          — JSON-RPC 2.0 MCP tool calls AND notifications
//                     Requests (has "id")    → JSON-RPC response (200 OK)
//                     Notifications (no "id") → 202 Accepted, empty body
//                     (JSON-RPC 2.0 §5 + MCP Streamable HTTP spec §3.2)
//   GET  /          — Server-Sent Events stream (MCP Streamable HTTP §3.3)
//   GET  /events    — Alias for GET / (backward compat for existing clients)
...
/// POST / — MCP Streamable HTTP §3.2 compliant handler.
///
/// JSON-RPC 2.0 §5 distinguishes:
///   Request      — has "id" field → dispatch and return JSON response (200 OK).
///   Notification — NO "id" field  → discard silently, return 202 Accepted (empty body).
///
/// Responding to a Notification with any JSON breaks MCP clients
/// (Claude Desktop's Zod validator rejects unexpected responses).
...
mod tests {
    ...
    #[tokio::test]
    async fn notification_returns_202_empty_body() {
        let (app, _) = test_app().await;
    ...
    #[tokio::test]
    async fn get_root_returns_sse_stream() {
        let (app, _) = test_app().await;
        let req = Request::builder()
            .method(Method::GET)
            .uri("/")
    ...
    #[tokio::test]
    async fn events_alias_returns_sse_stream() {
        let (app, _) = test_app().await;
        let req = Request::builder()
            .method(Method::GET)
            .uri("/events")
